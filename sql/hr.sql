...
--
-- Host: localhost:8889
-- Generation Time: Sep 21, 2020 at 01:23 AM
...
INSERT INTO `departments` (`department_id`, `department_name`, `manager_id`, `location_id`, `name`) VALUES
...
('DEV_DEV', 'Software developer', '4200', '9000'),
...
--
-- Stand-in structure for view `v_current_job`
-- (See below for the actual view)
--
CREATE TABLE `v_current_job` (
`employee_id` int(11) unsigned
,`first_name` varchar(20)
,`last_name` varchar(25)
,`department_name` varchar(30)
...
--
-- Stand-in structure for view `v_departments_by_country`
-- (See below for the actual view)
--
CREATE TABLE `v_departments_by_country` (
`department_id` int(11) unsigned
,`department_name` varchar(30)
,`city` varchar(30)
,`country_name` varchar(40)
...
--
-- Stand-in structure for view `v_employee_info`
-- (See below for the actual view)
--
CREATE TABLE `v_employee_info` (
`employee_id` int(11) unsigned
,`first_name` varchar(20)
,`last_name` varchar(25)
);
...
--
-- Stand-in structure for view `v_job_history`
-- (See below for the actual view)
--
CREATE TABLE `v_job_history` (
`employee_id` int(11) unsigned
,`first_name` varchar(20)
,`last_name` varchar(25)
,`department_name` varchar(30)
...
--
-- Structure for view `v_current_job`
--
DROP TABLE IF EXISTS `v_current_job`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_current_job`  AS  select `e`.`employee_id` AS `employee_id`,`e`.`first_name` AS `first_name`,`e`.`last_name` AS `last_name`,`d`.`department_name` AS `department_name`,`j`.`job_title` AS `job_title`,`j`.`min_salary` AS `min_salary`,`j`.`max_salary` AS `max_salary` from ((`employees` `e` join `departments` `d`) join `jobs` `j`) where ((`e`.`department_id` = `d`.`department_id`) and (`e`.`job_id` = `j`.`job_id`)) ;

-- --------------------------------------------------------

--
-- Structure for view `v_departments_by_country`
--
DROP TABLE IF EXISTS `v_departments_by_country`;

CREATE ALGORITHM=UNDEFINED DEFINER=`azur`@`%` SQL SECURITY DEFINER VIEW `v_departments_by_country`  AS  select `d`.`department_id` AS `department_id`,`d`.`department_name` AS `department_name`,`l`.`city` AS `city`,`c`.`country_name` AS `country_name`,`r`.`region_name` AS `region_name` from (((`departments` `d` join `locations` `l` on((`d`.`location_id` = `l`.`location_id`))) join `countries` `c` on((`c`.`country_id` = `l`.`country_id`))) join `regions` `r` on((`r`.`region_id` = `c`.`region_id`))) ;

-- --------------------------------------------------------

--
-- Structure for view `v_employee_info`
--
DROP TABLE IF EXISTS `v_employee_info`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_employee_info`  AS  select `e`.`employee_id` AS `employee_id`,`e`.`first_name` AS `first_name`,`e`.`last_name` AS `last_name` from `employees` `e` ;

-- --------------------------------------------------------

--
-- Structure for view `v_job_history`
--
DROP TABLE IF EXISTS `v_job_history`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_job_history`  AS  select `e`.`employee_id` AS `employee_id`,`e`.`first_name` AS `first_name`,`e`.`last_name` AS `last_name`,`d`.`department_name` AS `department_name`,`j`.`job_title` AS `job_title`,`j`.`min_salary` AS `min_salary`,`j`.`max_salary` AS `max_salary` from (((`employees` `e` join `job_history` `jh` on((`jh`.`employee_id` = `e`.`employee_id`))) join `departments` `d` on((`jh`.`department_id` = `d`.`department_id`))) join `jobs` `j` on((`jh`.`job_id` = `j`.`job_id`))) ;
